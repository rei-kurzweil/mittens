...
1288  avatar_control.tick                  → reads neck_parent.matrix_world; emits UpdateTransform(splice_head)
1289  queue.flush                          → executes UpdateTransform(splice_head):
        transform_changed(splice_head)
...
| `splice_head` TRS | AvatarControlSystem.tick_one | queue.flush at 1289, AFTER skinned_mesh.tick |
| head/hair bone world matrices | transform_changed(splice_head) | same flush |

The SkinnedMesh at 1230 sees body transforms from the **current frame** (pipeline ran this frame)
but splice_head from the **previous frame** (ACS hasn't run yet). This is a 1-frame offset between
body and head.
...
| `splice_head` TRS | AvatarControlSystem.tick_one | queue.flush at 1289 (same value as previous frame, since driven_t didn't change) |
...
active tick:
  ...
  ACS emits UpdateTransform(splice_head)  ← head rotation
...
idle tick:
  ...
  ACS still emits UpdateTransform(splice_head) — SAME VALUE
...
active tick:
  ...
  ACS emits UpdateTransform(splice_head) at flush 1289
...
idle tick:
  ...
  ACS emits UpdateTransform(splice_head) — same value (nothing changed)
```
...
On the last active frame (frame N): skinned_mesh sees body_yaw_N and splice_head_N-1.
On frame N+1 (first idle): skinned_mesh sees body_yaw_N (body pipeline didn't re-run) and
splice_head_N. Body and head are now from different "generations" — body from frame N's
...
and splice_head_N − splice_head_N-1 differ.
...
When splice_head applies a pitch rotation R, J_Bip_C_Neck's world position shifts: the bone
...
that splice_head is applying a larger rotation than it should. Possible causes:
...
5. **Head bone over-movement**: check if the pivot for head deformation is neck_parent vs.
   ...
   splice_head was inserted), they may be misaligned from the authored rest pose.
