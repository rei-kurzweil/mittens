...
- `RayCastCoords`: delta comes from consecutive ray-hit points; requires “still hitting the handle”.
- `ScreenSpaceCoords`: after `DragStart`, continue producing deltas by intersecting the **current cursor ray** against a captured plane; does *not* require continued handle hits.
...
## Why `DragCoordinateSource` was placed in Gestures (not UserInput)

`UserInput`/`InputState` is currently a thin adapter from **winit** to a per-frame snapshot.

`DragCoordinateSource` is not a property of the mouse device; it’s a property of the **gesture interpretation**:
...
- `DragCoordinateSource::RayCastCoords` (or a future “controller-space” source)
