# Render to texture: Layer A

This document describes the **implemented** Layer A runtime-texture bridge used by the bloom debug views.

It is intentionally narrower than the broader draft in [docs/draft/render-to-texture.md](docs/draft/render-to-texture.md):

- it covers exposing renderer-owned pass outputs as sampleable scene textures
- it does **not** define general capture cameras, mirrors, portals, or a first-class `RenderImageHandle`
...
## 3. ECS/runtime wiring

### `TextureComponent`

`TextureComponent` now supports:
...
- the draft still describes the likely long-term abstraction, but this document is the source of truth for the implementation that exists now