...
fn main() {
    ...
    let _ = universe.world.add_child(input, cam_transform);

    let camera = universe.world.add_component(Camera3DComponent::new());
    let _ = universe.world.add_child(cam_transform, camera);
    ...
    for (kind, nodes) in layers {
        ...
        for (i, node) in nodes.iter().enumerate() {
            ...
            let _ = universe.world.add_child(node_t, node_txt);
            ...
            let _ = universe.world.add_child(node_txt, node_txt_filter);

            universe
                .world
                .init_component_tree(node_t, &mut universe.command_queue);
        }
    }
    ...
    for edge in &graph.edges {
        ...
        let _ = universe.world.add_child(edge_root, box_t);
        ...
        let _ = universe.world.add_child(box_t, edge_r);
        let _ = universe.world.add_child(edge_r, edge_c);
        ...
        let _ = universe.world.add_child(edge_root, label_t);
        ...
        let _ = universe.world.add_child(label_t, label_txt);

        let label_filter = universe
            .world
            .add_component(TextureFilteringComponent::nearest());
        let _ = universe.world.add_child(label_txt, label_filter);

        universe
            .world
            .init_component_tree(edge_root, &mut universe.command_queue);
    }
