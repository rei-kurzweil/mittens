...
ED.active() {
    // The attachment system suspends this input's translation channel while
    // Bisket is mounted, but deliberately leaves its mouse/arrow look live.
    // These handlers then give W/A/S/D to the mounted car instead.
    let vehicle_state = {
        mounted = false
        w_held = false
        a_held = false
    ...
    // Keep the desktop vehicle's muzzle effect identical to the XR fixture:
    // its origin is measured after the car GLTF imports, then a replayable
    // one-shot animation lights the flash and beam along local -Z.
    ...
    on(car_root, "MountStarted", fn(event) {
        ...
        // Require fresh vehicle input after mounting rather than inheriting a
        // key that was held while Bisket was still walking.
    ...
    // Keyboard signals are global rather than attached to Input{}. Input owns
    // pedestrian pose driving; this explicit held-key state owns the mounted
    // vehicle layer. KeyUp is also delivered on focus/capture cleanup, so it
    // cannot leave the car driving after the window loses focus.
    ...
    on_global("FrameTick", fn(event) {
        if vehicle_state.mounted {
            let steering = 0.0
            let throttle = 0.0
            if vehicle_state.a_held {
                steering = steering - 1.0
            }
            if vehicle_state.d_held {
                steering = steering + 1.0
            }
            if vehicle_state.w_held {
                throttle = throttle + 1.0
            }
            if vehicle_state.s_held {
                throttle = throttle - 1.0
            }
