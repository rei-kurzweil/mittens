...
fn main() {
    ...
    let out = meow_meow::MeowMeowRunner::eval_with_world(
        src, &mut universe.world, &mut universe.systems.rx, &mut universe.command_queue,
    );
    for e in &out.errors { eprintln!("[mms-err] {e}"); }
    println!("intents: {}", out.intents.len());
    for c in universe.world.all_components() {
        if let Some(label) = universe.world.component_label(c) {
            {
                println!("  comp {:?} type={:?} name={:?}", c, universe.world.component_name(c), label);
            }
        }
    }
}
