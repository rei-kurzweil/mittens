...
fn main() {
    ...
    let output = meow_meow::MeowMeowRunner::eval(include_str!("vtuber-desktop-first-person.mms"));

    for error in &output.errors {
        eprintln!("[mms] {error}");
    }
    println!("[mms] {} intent(s) from vtuber-desktop-first-person.mms", output.intents.len());
