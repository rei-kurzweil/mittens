...
pub enum KinematicResponseMode {
...
/// Opt-in kinematic collision response behavior for a transform subtree.
///
/// Attach this as a direct child of a `TransformComponent`. If that transform also has a
/// `CollisionComponent` child, the KinematicResponseSystem may move the transform to resolve
/// overlaps against static collision objects.
#[derive(Debug, Clone)]
pub struct KinematicResponseComponent {
    pub enabled: bool,
    pub mode: KinematicResponseMode,
...
impl KinematicResponseComponent {
    pub fn new(mode: KinematicResponseMode) -> Self {
        Self {
            enabled: true,
            mode,
            max_iterations: 6,
    ...
    pub fn slide() -> Self {
        let mut c = Self::new(KinematicResponseMode::Slide);
        c.push_strength = 0.0;
        c.friction = 0.0;
        c.max_speed = 0.0;
        c
    ...
    pub fn push() -> Self {
        Self::new(KinematicResponseMode::Push)
    }
...
impl Default for KinematicResponseComponent {
    fn default() -> Self {
        Self::slide()
    }
}

impl Component for KinematicResponseComponent {
    ...
    fn init(&mut self, queue: &mut crate::engine::ecs::CommandQueue, component: ComponentId) {
        queue.queue_register_kinematic_response(component);
    }

    fn cleanup(&mut self, queue: &mut crate::engine::ecs::CommandQueue, component: ComponentId) {
        queue.queue_remove_kinematic_response(component);
    }
    ...
    fn encode(&self) -> std::collections::HashMap<String, serde_json::Value> {
        ...
        map.insert(
            ...
            serde_json::json!(match self.mode {
                KinematicResponseMode::Slide => "slide",
                KinematicResponseMode::Push => "push",
            }),
        );
        map.insert(
    ...
    fn decode(
        ...
        if let Some(enabled) = data.get("enabled") {
            self.enabled = serde_json::from_value(enabled.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.enabled: {e}"))?;
        }
        if let Some(mode) = data.get("mode") {
            let mode_str: String = serde_json::from_value(mode.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.mode: {e}"))?;
            self.mode = match mode_str.as_str() {
                "slide" => KinematicResponseMode::Slide,
                "push" => KinematicResponseMode::Push,
                other => return Err(format!("Unknown kinematic response mode: {other}")),
            };
        }
        if let Some(max_it) = data.get("max_iterations") {
            self.max_iterations = serde_json::from_value(max_it.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.max_iterations: {e}"))?;
        }
        if let Some(eps) = data.get("push_out_epsilon") {
            self.push_out_epsilon = serde_json::from_value(eps.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.push_out_epsilon: {e}"))?;
        }

        if let Some(v) = data.get("push_strength") {
            self.push_strength = serde_json::from_value(v.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.push_strength: {e}"))?;
        }
        if let Some(v) = data.get("friction") {
            self.friction = serde_json::from_value(v.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.friction: {e}"))?;
        } else if let Some(v) = data.get("velocity_damping_per_sec") {
            // Back-compat: old name for friction-like damping.
            self.friction = serde_json::from_value(v.clone()).map_err(|e| {
                format!("Failed to decode kinematic_response.velocity_damping_per_sec: {e}")
            })?;
        }
        if let Some(v) = data.get("friction_y") {
            self.friction_y = serde_json::from_value(v.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.friction_y: {e}"))?;
        }
        if let Some(v) = data.get("max_speed") {
            self.max_speed = serde_json::from_value(v.clone())
                .map_err(|e| format!("Failed to decode kinematic_response.max_speed: {e}"))?;
        }
        Ok(())
    }
}
