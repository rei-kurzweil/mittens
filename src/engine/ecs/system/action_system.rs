...
pub(crate) fn register_action(world: &mut World, emit: &mut dyn SignalEmitter, component: ComponentId) {
    ...
    while let Some(parent) = world.parent_of(cur) {
        if world.get_component_by_id_as::<KeyframeComponent>(parent).is_some() {
            return;
        }
        cur = parent;
    }
