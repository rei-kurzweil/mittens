...
use crate::engine::ecs::component::{
    CollisionComponent, CollisionMode, CollisionShape, CollisionShapeComponent, KinematicResponseComponent,
    KinematicResponseMode, RenderableComponent, TransformComponent,
};
...
pub struct KinematicResponseSystem {
    responders: Vec<ComponentId>,
}

impl KinematicResponseSystem {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn register_kinematic_response(&mut self, component: ComponentId) {
        if !self.responders.iter().any(|c| *c == component) {
            self.responders.push(component);
        }
    }

    pub fn remove_kinematic_response(&mut self, component: ComponentId) {
    ...
    pub fn tick_with_queue(
        ...
        for response_cid in responder_ids {
            let Some(response) = world.get_component_by_id_as::<KinematicResponseComponent>(response_cid) else {
                continue;
            };
            if !response.enabled {
                continue;
            ...
            let Some(transform_cid) = world.parent_of(response_cid) else {
                continue;
            };
            if world
            ...
            if response.mode == KinematicResponseMode::Slide && statics.is_empty() {
                continue;
            }
            ...
            if response.mode == KinematicResponseMode::Push {
                ...
                for &static_cid in statics.iter() {
                    ...
                    if response.mode == KinematicResponseMode::Push {
                        ...
                        if axis == 1 {
                            ...
                            // Side walls: bounce in X/Z.
                            // If velocity is into the wall, reflect it.
        ...
        for (response_cid, transform_cid, new_local_translation, new_velocity) in pending_updates {
            if let Some(r) = world.get_component_by_id_as_mut::<KinematicResponseComponent>(response_cid) {
