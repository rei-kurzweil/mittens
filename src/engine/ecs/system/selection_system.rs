use crate::engine::ecs::component::{SelectionComponent, SelectionEntry, TextComponent};
...
fn handle_selection_click(
    ...
    let Some(item_id) = find_selected_subtree_under_selection(world, selection_root, renderable)
        ...
        if let Some(rec) = world.get_component_record(renderable) {
            println!(
                "[selection] no subtree found under selection for renderable name={:?} type={}",
                rec.name, rec.component_type
            );
        }
        return;
    };
    ...
    if was_selected && !is_selected_now {
        set_asset_item_selected_color(world, emit, item_id, false);
        remove_selection_highlight(world, emit, item_id);
...
mod tests {
    ...
    use crate::engine::ecs::component::SelectionMode;
    ...
    fn selection_system_click_updates_selection_state() {
        ...
        println!(
            "text_under_wrapper={:?}",
            world.find_all_components(wrapper, "Text")
        );
        println!(
            "text_under_selection={:?}",
            world.find_all_components(selection_root, "Text")
        );
    ...
    fn selection_system_multiple_mode_toggles_membership() {
        ...
        let items = world.find_all_components(selection_root, "[name='asset_item']");
