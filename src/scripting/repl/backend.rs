    ...
    match kind {
        HostCallKind::Spawn(ce) => {
            crate::scripting::component_registry::with_live_render_assets(render_assets, || {
                crate::scripting::component_registry::spawn_tree(&ce, None, world, emit)
            })
        ...
        HostCallKind::Register(ce) => {
            crate::scripting::component_registry::with_live_render_assets(render_assets, || {
                crate::scripting::component_registry::spawn_tree_uninitialized(&ce, world, emit)
            })
            .map(HostValue::ComponentId)
            .unwrap_or_else(|e| {
...
fn dump_value(value: &Value, world: &ecs::World) {
    match selected_ids(value, world) {
        Ok(ids) => {
            for id in ids {
                match crate::scripting::component_registry::subtree_to_ce_ast(world, id) {
                    Ok(ce) => println!("{}", crate::scripting::unparser::unparse_component(&ce)),
